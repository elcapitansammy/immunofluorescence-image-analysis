...
def save_average_intensities_to_csv(average_intensities1, average_intensities2, average_intensities3, areas, name):
    ...
    for i in range(len(average_intensities1)):
        if average_intensities2:
            data.append({'Index': i, 'Average Intensity red': average_intensities1[i],
                         'Average Intensity green': average_intensities2[i],
                         'Average Intensity blue': average_intensities3[i], 'Area': areas[i]})
        else:
            data.append({'Index': i, 'Average Intensity red': average_intensities1[i], 'Average Intensity green': 0,
                         'Average Intensity blue': average_intensities3[i], 'Area': areas[i]})
...
def calculate_and_save(image_path, name):
    ...
    save_average_intensities_to_csv(average_intensities_D_red, average_intensities_D_green, average_intensities_D_blue,
                                    areas, name)
...
blue_masks, areas = create_masks('B1_Clk-_PC_x32_LM_red_Fig1_DAPI.tif', 200)
image = cv2.imread('B1_Clk-_PC_x32_LM_red_Fig1_AF647.tif')
average_intensities_D_red = calculate_average_intensity(image, blue_masks, 'red')
print(average_intensities_D_red)
